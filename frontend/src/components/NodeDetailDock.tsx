...
/* ── Section data ────────────────────────────────────────────── */
interface DragItem  { name: string; emoji: string; desc: string; key: string; }
...
const SECTIONS: Section[] = [
  ...
  } as ClickSection,
  {
    ...
    items: [
      ...
      { name: 'PostgreSQL',  emoji: '🐘', desc: 'Inbound TCP 5432',       extra: { direction:'inbound',  protocol:'tcp', portRange:'5432',  cidr:'10.0.0.0/8', action:'allow' } },
      { name: 'Redis',       emoji: '🔴', desc: 'Inbound TCP 6379',       extra: { direction:'inbound',  protocol:'tcp', portRange:'6379',  cidr:'10.0.0.0/8', action:'allow' } },
      { name: 'All Outbound',emoji: '↗️',  desc: 'Allow all egress',       extra: { direction:'outbound', protocol:'any', portRange:'0-65535',cidr:'0.0.0.0/0', action:'allow' } },
  ...
  if (section.clickOnly) {
    ...
    return (
      <div
        ...
        title={ci.desc}
      >
        <div className="dock-item-icon" style={{ background: `${section.color}18`, color: section.color, fontSize: '1.3rem' }}>
          {ci.emoji}
        </div>
